...
export default function NetworkMenu() {
  return (
    <box class="NetworkMenuCard" orientation={Gtk.Orientation.VERTICAL} spacing={10}>
      {/* 1. Header (Orange Badge + "Wi-Fi" + Spinner/Refresh) */}
      ...
      >
        <box class="network-list" orientation={Gtk.Orientation.VERTICAL} spacing={4}>
          {createComputed(() => {
            const list = Network.networks()
            if (list.length === 0) {
              return (
                <box class="empty-networks" halign={Gtk.Align.CENTER} valign={Gtk.Align.CENTER}>
                  <label class="empty-text" label={Network.isScanning() ? "Scanning for networks..." : "No networks found"} />
                </box>
              )
            }

            return list.slice(0, 12).map((ap: WifiAccessPoint) => (
              <button
